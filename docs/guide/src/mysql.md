...
`RETURNING` clauses on mutations, so a value the database computes can only
reach the caller through some other mechanism. MySQL has one:
`LAST_INSERT_ID()` reports the auto-increment value the most recent `INSERT`
generated on the same connection. For a one-row insert that value names the
row exactly, so [`Model::create()`](./creating-records.md) returns a populated
model the same way it does on other backends.

That covers one row. `LAST_INSERT_ID()` reports a single value, and the IDs of
a multi-row insert cannot be derived from it: MySQL's
`auto_increment_increment` can be larger than one, and under the default
`innodb_autoinc_lock_mode=2` concurrent inserts interleave their allocations.
So Toasty rejects a multi-row insert whose generated IDs it would have to
report, with an `unsupported_feature` error naming the column:
...
// Rejected on MySQL: `users.id` is generated by the database, so each
// returned model needs a value only the database can report.
User::create_many()
...
Three ways to write the same thing on MySQL:

- Insert the rows one at a time. Each insert is a separate statement, so each
  generated ID is reported exactly.
- Set the key from the application — a `uuid::Uuid` key, or any key whose
  value Toasty knows before the statement runs. Toasty then derives the
  returned models from the input and the insert stays a single multi-row
  statement.
- Use a backend that supports `RETURNING`, such as PostgreSQL or SQLite.

The same rule applies to any other value the statement would need back from
the database, not just an auto-increment key.
