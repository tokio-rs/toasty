...
#### 2. Getting Started
- Add toasty to Cargo.toml (plus tokio)
...
#### 5. Schema Management
- `db.push_schema()` — create tables and indexes from registered models
- `db.reset_db()` — drop and recreate the database (development only)
- When to call each method
- How model definitions map to DDL (tables, columns, indexes, constraints)
...
#### 17. Sorting, Limits, and Pagination
...
Write chapters in order (1-21). Each chapter is a separate `.md` file in `docs/guide/src/`. Update `SUMMARY.md` as chapters are added.
