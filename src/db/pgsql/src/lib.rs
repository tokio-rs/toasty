...
impl PostgreSQL {
    ...
    pub async fn from_postgres_config<T>(config: Config, tls: T) -> Result<Self>
    where
        T: MakeTlsConnect<Socket> + 'static,
        T::Stream: Send,
    {
...
fn postgres_to_toasty(index: usize, row: &Row, column: &Column) -> stmt::Value {
    ...
    if column.type_() == &Type::TEXT {
        let value = row.get::<usize, Option<String>>(index);
        value.map(stmt::Value::String).unwrap_or(stmt::Value::Null)
    } else {
        todo!("{:#?}", column.type_());
    }
}
