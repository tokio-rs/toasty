...
/// Let/Map scopes. Read-only counterpart of
/// [`visit_mut::walk_expr_scoped_mut`](super::visit_mut::walk_expr_scoped_mut).
///
/// For each node, calls `f(expr, scope_depth)`:
/// - If `f` returns `true`, recursion into children continues.
/// - If `f` returns `false`, children are skipped.
///
/// Scope depth rules:
...
pub fn walk_expr_scoped<F>(expr: &Expr, scope_depth: usize, mut f: F)
where
    F: FnMut(&Expr, usize) -> bool,
{
    ...
    impl<F> Visit for ScopedWalk<'_, F>
    where
        F: FnMut(&Expr, usize) -> bool,
    {
        fn visit_expr(&mut self, node: &Expr) {
        ...
        fn visit_expr_map(&mut self, node: &ExprMap) {
            self.visit_expr(&node.base);
            self.scope_depth += 1;
            self.visit_expr(&node.map);
            self.scope_depth -= 1;
        }
    }
