...
use super::{
    ...
    ExprStmt, ExprVariant, Filter, FuncCount, FuncJsonExtract, FuncLastInsertId, FuncUnnest,
    Include, Insert, InsertTarget, Join, JoinOp, Limit, LimitCursor, LimitOffset, Node, OrderBy,
...
pub trait Visit {
    ...
    /// Visits an [`ExprIncoming`] node.
    ///
    /// The default implementation delegates to [`visit_expr_incoming`].
    fn visit_expr_incoming(&mut self, i: &ExprIncoming) {
        visit_expr_incoming(self, i);
    }
    ...
    /// Visits an [`ExprVariant`] node.
    ///
    /// The default implementation delegates to [`visit_expr_variant`].
    fn visit_expr_variant(&mut self, i: &ExprVariant) {
        visit_expr_variant(self, i);
    }
...
impl<V: Visit> Visit for &mut V {
    ...
    fn visit_expr_incoming(&mut self, i: &ExprIncoming) {
        Visit::visit_expr_incoming(&mut **self, i);
    }
    ...
    fn visit_expr_variant(&mut self, i: &ExprVariant) {
        Visit::visit_expr_variant(&mut **self, i);
    }
    ...
    V: Visit + ?Sized,
{
    match node {
        ...
        Expr::Incoming(expr) => v.visit_expr_incoming(expr),
        ...
        Expr::Variant(expr) => v.visit_expr_variant(expr),
...
/// Default traversal for [`ExprIncoming`] nodes. This is a leaf node with no children to visit.
pub fn visit_expr_incoming<V>(_v: &mut V, _node: &ExprIncoming)
where
    V: Visit + ?Sized,
{
}
...
/// Default traversal for [`FuncUnnest`] nodes. Visits the array argument.
...
/// Default traversal for [`ExprVariant`] nodes. Visits the base expression.
pub fn visit_expr_variant<V>(v: &mut V, node: &ExprVariant)
where
    V: Visit + ?Sized,
{
    v.visit_expr(&node.base);
