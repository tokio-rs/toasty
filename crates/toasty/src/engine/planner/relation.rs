...
impl Planner<'_> {
            ...
            self.relation_step(field, |planner| {
                ...
                if nullify {
                    ...
                    if field.nullable {
                        ...
                        planner.plan_stmt_raw(&Context::default(), stmt.into())?;
                    } else {
    ...
    pub(super) fn plan_mut_belongs_to_stmt(
        ...
        match stmt {
            stmt::Statement::Insert(mut insert) => {
                ...
                let insertion_output = self
                    .plan_stmt_raw(&Context::default(), insert.into())?
                    .unwrap();
        ...
        self.relation_step(field, |planner| {
            ...
            if let Some(pair) = belongs_to.pair.map(|pair| planner.schema.app.field(pair)) {
                if pair.ty.is_has_one() && !pair.nullable {
                    ...
                    planner.plan_stmt_raw(&Context::default(), delete.into())?;
                }
            }
    ...
    fn plan_mut_has_many_value(
        ...
        if op.is_remove() {
            ...
            let out = self.plan_stmt_raw(&Context::default(), stmt.into())?;
            assert!(out.is_none());
        }
    ...
    fn plan_mut_has_many_value_remove(
        ...
        if pair.nullable {
            ...
            let out = self.plan_stmt_raw(&Context::default(), selection.delete().into())?;
            assert!(out.is_none());
        }
    ...
    pub(super) fn plan_mut_has_one_nullify(
        ...
        if self.schema.app.field(has_one.pair).nullable {
            ...
            let out = self.plan_stmt_raw(&Context::default(), pair_scope.delete().into())?;
            assert!(out.is_none());
        }
