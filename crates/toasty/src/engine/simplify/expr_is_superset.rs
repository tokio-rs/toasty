...
impl Simplify<'_> {
    /// Rewrite `IsSuperset(path, [v1, v2, …])` into `AND(any_op(v1, Eq, path), …)`
    ...
    pub(super) fn simplify_expr_is_superset(
        &self,
        expr: &mut stmt::ExprIsSuperset,
    ) -> Option<stmt::Expr> {
        ...
        // Empty-rhs has been folded to `true` upstream by `fold::expr_is_superset`.
        ...
        let operands: Vec<stmt::Expr> = values
            ...
            .collect();
        Some(stmt::Expr::and_from_vec(operands))
    }
}
