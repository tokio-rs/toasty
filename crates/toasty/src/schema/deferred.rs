...
/// `Load` adapter that decodes a JSON-encoded column into `T`.
///
/// Used by the per-field lazy-load accessor generated for fields annotated
/// with both `#[serialize(json)]` and `#[deferred]`. The accessor returns
/// `Statement<LoadJson<T>>`, whose `Output = T`, so `.exec()` yields the
/// deserialized value directly.
///
/// `T` may itself be `Option<U>`; the impl maps a `Null` column to the JSON
/// literal `null` before deserializing, which `Option<U>` decodes as `None`.
/// Non-`Option` `T` instead surface a deserialization error on `Null`, which
/// is the right behavior for a non-nullable column.
#[doc(hidden)]
#[cfg(feature = "serde")]
pub struct LoadJson<T>(std::marker::PhantomData<T>);

#[cfg(feature = "serde")]
impl<T> Load for LoadJson<T>
where
