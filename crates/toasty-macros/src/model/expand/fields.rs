...
impl Expand<'_> {
        ...
        quote!(
            ...
            impl<#generics> #field_struct_ident<#generics> {
                #vis fn in_query(self, rhs: impl #toasty::IntoStatement<Returning = #toasty::List<#model_ident>>) -> #toasty::stmt::Expr<bool> {
                    // Optional relations still test membership in a query of models.
                    let path: #toasty::core::stmt::Path = self.path.into();
                    #toasty::stmt::Expr::from_untyped(
                        #toasty::core::stmt::Expr::in_subquery(
                            path.into_stmt(),
                            rhs.into_statement().into_untyped().into_query_unwrap(),
                        )
                    )
                }
    ...
    fn expand_field_struct_comparison_methods(&self, target_ty: &TokenStream) -> TokenStream {
        ...
        let methods = names.iter().map(|name| {
            ...
            quote! {
                #vis fn #method_ident(self, rhs: impl #toasty::IntoExpr<#target_ty>) -> #toasty::stmt::Expr<bool> {
                    use #toasty::IntoExpr;
