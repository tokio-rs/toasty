...
impl Expand<'_> {
        ...
        self.model.fields.iter().enumerate().map(|(field_index, field)| {
            ...
            FieldTy::Primitive(_) if field.attrs.serialize.is_some() => {
                ...
                if serialize_attr.nullable {
                    quote! {
                        #vis fn #field_ident(mut self, #field_ident: #arg_ty) -> Self {
                            self.#set_field_ident(#field_ident);
                            self
                        }

                        #vis fn #set_field_ident(&mut self, #field_ident: #arg_ty) -> &mut Self {
                            let projection = #projection;
                            match &#field_ident {
                                Some(v) => {
    ...
    fn expand_reload_match_arms(&self) -> TokenStream {
        ...
        self.model.fields.iter().enumerate().map(|(offset, field)| {
            ...
            match &field.ty {
                FieldTy::Primitive(_) if field.attrs.serialize.is_some() => {
                    let mut assign = self.expand_serialize_decode(field);
                    if field.attrs.deferred {
                        assign = self.wrap_in_deferred(field, assign);
                    }
                    quote! {
                        #i => {
