            ...
            Flavor::Mysql => {
                let new_column_def = ColumnDef {
                    ...
                    comment: self
                        ...
                        .new_comment
                        .clone()
                        .unwrap_or_else(|| self.column_def.comment.clone()),
...
impl ToSql for &stmt::CommentOn {
    fn to_sql(self, f: &mut super::Formatter<'_>) {
        match f.serializer.flavor {
            Flavor::Postgresql => {
                let comment = match &self.comment {
                    Some(comment) => stmt::Value::String(comment.clone()),
                    None => stmt::Value::Null,
                };

                match &self.target {
                    stmt::CommentTarget::Table(table) => {
                        fmt!(f, "COMMENT ON TABLE " table " IS ");
                        (&comment).to_sql(f);
                    }
                    stmt::CommentTarget::Column { table, column } => {
                        fmt!(f, "COMMENT ON COLUMN " table "." column " IS ");
                        (&comment).to_sql(f);
                    }
                }
            }
            Flavor::Mysql | Flavor::Sqlite => {
                panic!("standalone COMMENT ON statements are only supported on PostgreSQL")
            }
        }
    }
}
...
impl ToSql for &stmt::AlterTable {
    fn to_sql(self, f: &mut super::Formatter<'_>) {
        match &self.action {
            ...
            stmt::AlterTableAction::SetComment(comment) => match f.serializer.flavor {
                Flavor::Mysql => {
                    let comment = match comment {
                        Some(comment) => stmt::Value::String(comment.clone()),
                        None => stmt::Value::String(String::new()),
                    };
                    fmt!(f, "ALTER TABLE " self.name " COMMENT = ");
                    (&comment).to_sql(f);
                }
                Flavor::Postgresql | Flavor::Sqlite => {
                    panic!("ALTER TABLE COMMENT is only supported on MySQL")
                }
            },
        }
    }
...
impl ToSql for &stmt::Statement {
    fn to_sql(self, f: &mut super::Formatter<'_>) {
        match self {
            ...
            stmt::Statement::CommentOn(stmt) => stmt.to_sql(f),
