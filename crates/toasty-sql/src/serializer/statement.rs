    ...
    fn to_sql<P: Params>(self, cx: &ExprContext<'_>, f: &mut super::Formatter<'_, P>) {
        ...
        match f.serializer.flavor {
            Flavor::Postgresql => match &self.changes {
                ...
                } => fmt!(&cx, f, "ALTER TABLE " table_name " RENAME COLUMN " self.column_def.name.as_str() " TO " name.as_str()),
                AlterColumnChanges {
                    new_name: None,
                    new_ty: Some(ty),
                    new_not_null: None,
                    new_auto_increment: None,
                } => fmt!(&cx, f, "ALTER TABLE " table_name " ALTER COLUMN " self.column_def.name.as_str() " TYPE " ty),
                AlterColumnChanges {
                    new_name: None,
                    new_ty: None,
                    new_not_null: Some(true),
                    new_auto_increment: None,
                } => fmt!(&cx, f, "ALTER TABLE " table_name " ALTER COLUMN " self.column_def.name.as_str() " SET NOT NULL"),
                AlterColumnChanges {
                    new_name: None,
                    new_ty: None,
                    new_not_null: Some(false),
                    new_auto_increment: None,
                } => fmt!(&cx, f, "ALTER TABLE " table_name " ALTER COLUMN " self.column_def.name.as_str() " DROP NOT NULL"),
                AlterColumnChanges {
                    new_name: None,
                    new_ty: None,
                    new_not_null: None,
                    new_auto_increment: Some(true),
                } => fmt!(&cx, f, "ALTER TABLE " table_name " ALTER COLUMN " self.column_def.name.as_str() " ADD GENERATED BY DEFAULT AS IDENTITY"),
                AlterColumnChanges {
                    new_name: None,
                    new_ty: None,
                    new_not_null: None,
                    new_auto_increment: Some(false),
                } => fmt!(&cx, f, "ALTER TABLE " table_name " ALTER COLUMN " self.column_def.name.as_str() " DROP IDENTITY"),
