...
fn in_range_ints_decode() {
    let value = json::list_from_str("[0, 127, -128]", &Type::I8).unwrap();
    assert_eq!(
        value,
...
fn negative_int_for_unsigned_type_is_decode_error() {
    let err = json::list_from_str("[-1]", &Type::U64).unwrap_err();
...
fn too_wide_int_is_decode_error() {
    let err = json::list_from_str("[300]", &Type::I8).unwrap_err();
    let msg = err.to_string();
    assert!(
...
fn unsigned_above_i64_max_decodes() {
    ...
    let value = json::list_from_str(&text, &Type::U64).unwrap();
