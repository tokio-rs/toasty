...
#[driver_test(requires(auto_increment))]
pub async fn auto_increment_with_single_association(test: &mut Test) -> Result<()> {
    #[derive(toasty::Model)]
    struct Parent {
        #[key]
        #[auto]
    ...
    for id in 1..4 {
        ...
        assert_eq!(parent.children.get()[0].id, id);
        assert_eq!(parent.children.get()[0].parent_id, id);
    }
...
pub async fn auto_increment_parent_with_batch_associations(test: &mut Test) -> Result<()> {
    #[derive(Debug, toasty::Model)]
    struct Parent {
        #[key]
        #[auto]
