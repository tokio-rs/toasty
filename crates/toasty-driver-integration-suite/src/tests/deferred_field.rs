...
// ---------- `#[serialize(json)] + #[deferred]` on a single field ----------
//
// The column is stored as JSON, the in-memory field is `Deferred<T>`, and
// `T` only implements `serde::{Serialize, Deserialize}` — never Toasty's
...
pub async fn deferred_serialize_create_returns_loaded(t: &mut Test) -> Result<()> {
    ...
    assert_eq!(&initial, created.payload.get());
...
pub async fn deferred_serialize_exec_lazy_loads_value(t: &mut Test) -> Result<()> {
    ...
    let read = Repository::filter_by_id(created.id).get(&mut db).await?;
    // The per-field accessor returns a Statement<LoadJson<Payload>>; .exec()
    // hands back the JSON-deserialized inner value directly.
    let payload: Payload = read.payload().exec(&mut db).await?;
...
pub async fn deferred_serialize_include_eager_loads_value(t: &mut Test) -> Result<()> {
    ...
    // `.include()` projects the JSON column into the SELECT; the model loader
    // peels the deferred envelope, JSON-decodes the inner String, and wraps
    // the resulting `Payload` back in a loaded `Deferred`.
    let read = Repository::filter_by_id(created.id)
        .include(Repository::fields().payload())
        .get(&mut db)
        .await?;
    assert!(!read.payload.is_unloaded());
    assert_eq!(&initial, read.payload.get());
...
pub async fn deferred_serialize_update_refreshes_loaded_value(t: &mut Test) -> Result<()> {
    ...
    assert!(!doc.payload.is_unloaded());
    assert_eq!(&next, doc.payload.get());
