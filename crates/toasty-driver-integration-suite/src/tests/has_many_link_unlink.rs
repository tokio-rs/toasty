...
pub async fn remove_add_single_relation_option_belongs_to(test: &mut Test) -> Result<()> {
    ...
    let todos_reloaded: Vec<_> = user.todos().collect(&db).await?;
    ...
    let u2 = User::create().todo(Todo::create()).exec(&db).await?;
    let u2_todos = u2.todos().collect::<Vec<_>>(&db).await?;

    // Try unlinking u2's todo via user. This should fail
    assert_err!(user.todos().remove(&db, &u2_todos[0]).await);

    // Reload u2's todo
    let u2_todo = Todo::get_by_id(&db, u2_todos[0].id).await?;
    assert_eq!(*u2_todo.user_id.as_ref().unwrap(), u2.id);

    // Link the TODO back up
    user.todos().insert(&db, &todos[0]).await?;
...
pub async fn add_remove_single_relation_required_belongs_to(test: &mut Test) -> Result<()> {
    ...
    // Create some TODOs
    let t1 = user.todos().create().exec(&db).await?;
    let t2 = user.todos().create().exec(&db).await?;
    let t3 = user.todos().create().exec(&db).await?;
    ...
    user.todos().remove(&db, &todos_reloaded[0]).await?;
...
pub async fn reassign_relation_required_belongs_to(test: &mut Test) -> Result<()> {
    ...
    let t1 = u1.todos().create().exec(&db).await?;

    // Associate the todo with user 2
    u2.todos().insert(&db, &t1).await?;

    // The TODO is no longer associated with user 1
    assert!(u1.todos().collect::<Vec<_>>(&db).await?.is_empty());

    // The TODO is assiated with user 2
    let todos = u2.todos().collect::<Vec<_>>(&db).await?;
...
pub async fn add_remove_multiple_relation_option_belongs_to(test: &mut Test) -> Result<()> {
    ...
    // Create some TODOs
    let t1 = Todo::create().exec(&db).await?;
    let t2 = Todo::create().exec(&db).await?;
    let t3 = Todo::create().exec(&db).await?;

    let ids = vec![t1.id, t2.id, t3.id];

    // Associate the todos with the user
    user.todos().insert(&db, &[t1, t2, t3]).await?;
