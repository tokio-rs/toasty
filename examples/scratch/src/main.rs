...
async fn main() -> toasty::Result<()> {
    ...
    let user = db::User::get_by_id(&db, &u1.id).await?;
    println!("USER = {user:#?}");

    // Find by email!
    println!("==> let user = User::find_by_email(&u1.email)");
    let mut user = db::User::get_by_email(&db, &u1.email).await?;
    println!("USER = {user:#?}");

    assert!(db::User::create()
        .name("John Dos")
        .email("john@example.com")
        .exec(&db)
        .await
    ...
    assert_eq!(
        db::User::filter_by_id(&user.id).get(&db).await?.name,
        user.name
    );

    // Load the user again
    let user = db::User::filter_by_id(&u1.id).get(&db).await?;
    ...
    // Delete user
    let user = db::User::get_by_id(&db, &u2.id).await?;
    user.delete(&db).await?;
    assert!(db::User::get_by_id(&db, &u2.id).await.is_err());

    // Create a batch of users
    db::User::create_many()
        .item(db::User::create().email("foo@example.com").name("User Foo"))
        .item(db::User::create().email("bar@example.com").name("User Bar"))
        .exec(&db)
        .await?;
    ...
    let mut user = db::User::create()
        ...
        .todo(db::Todo::create().title("Sleep"))
        .exec(&db)
        .await?;

    user.update()
        .todo(db::Todo::create().title("might delete later"))
        .exec(&db)
        .await?;
