...
impl Todo {
    ...
    pub const TITLE: Path<String> = Path::from_field_index::<Self>(1);
    pub const ORDER: Path<i64> = Path::from_field_index::<Self>(2);
    pub const USER: self::fields::User =
        self::fields::User::from_path(Path::from_field_index::<Self>(3));
    pub const USER_ID: Path<Id<super::user::User>> = Path::from_field_index::<Self>(4);
...
impl Model for Todo {
    ...
    fn load(mut record: Record<'_>) -> Result<Self, Error> {
        Ok(Todo {
            ...
            order: record[2].take().to_i64()?,
            user: BelongsTo::load(record[3].take())?,
            user_id: Id::from_untyped(record[4].take().to_id()?),
        })
    }
}
    ...
    pub async fn exec(self, db: &Db) -> Result<()> {
        ...
        for field in fields.iter() {
            match field.into_usize() {
                ...
                1 => self.model.title = into_iter.next().unwrap().to_string()?,
                2 => self.model.order = into_iter.next().unwrap().to_i64()?,
                3 => {
...
pub mod queries {
    ...
    impl super::Todo {
        pub fn find_by_user_id_and_id<'a>(
            user_id: impl stmt::IntoExpr<'a, Id<super::super::user::User>>,
            id: impl stmt::IntoExpr<'a, Id<Todo>>,
        ) -> FindByUserIdAndId<'a> {
            FindByUserIdAndId {
    ...
    pub struct FindManyByUserIdAndId<'a> {
        items: Vec<stmt::Expr<'a, (Id<super::super::user::User>, Id<Todo>)>>,
    }
