...
async fn ty_bool(test: &mut DbTest) {
    ...
    for &val in &test_values {
        ...
        let mut filter = std::collections::HashMap::new();
        filter.insert("id".to_string(), toasty_core::stmt::Value::from(created.id.clone()));
    ...
    for (id, expected_val) in created_records {
        let read = Foo::get_by_id(&db, &id).await.unwrap();
        assert_eq!(read.val, expected_val, "Multiple records test failed for bool: {}", expected_val);
    }
...
tests!(
    ty_bool, ty_i8, ty_i16, ty_i32, ty_i64, ty_isize, ty_u8, ty_u16, ty_u32, ty_u64, ty_usize, ty_str,
    ty_uuid
);
