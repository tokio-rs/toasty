...
async fn valid_integer_column_types(test: &mut DbTest) {
    ...
    struct Counter {
        ...
        // Valid: i32 with 4-byte integer storage
        #[column(type = integer(4))]
        count: i32,

        // Valid: i32 with 8-byte integer storage (oversized but valid)
        #[column(type = integer(8))]
        total: i32,

        // Valid: u64 with 8-byte unsigned integer storage
        #[column(type = unsignedinteger(8))]
        large_value: u64,
    }
...
async fn valid_optional_column_types(test: &mut DbTest) {
    ...
    struct User {
        ...
        // Valid: Option<i32> with integer storage
        #[column(type = integer(4))]
        age: Option<i32>,
    }
